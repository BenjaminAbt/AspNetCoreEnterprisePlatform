...
public class AzureLogAnalyticsWorkspaceController : SecurityPortalController
{
    private readonly IRequestUserIdentityTenantAuthAccessor _userIdentityTenantAuthAccessor;
    private readonly IEventDispatcher _eventDispatcher;
    private readonly IMapper _mapper;
    private readonly ILogger<AzureLogAnalyticsWorkspaceController> _logger;

    public AzureLogAnalyticsWorkspaceController(
        IRequestUserIdentityTenantAuthAccessor userIdentityTenantAuthAccessor,
        IEventDispatcher eventDispatcher, IMapper mapper,
        ILogger<AzureLogAnalyticsWorkspaceController> logger)
    {
    ...
    [HttpGet]
    [Route(MyDemoPlatformApiRoutes.SecurityPortal.AzureLogAnalyticsWorkspace.Stats)]
    public async Task<ActionResult<SecurityPortalAzureLogAnalyticsWorkspaceStatsModel>> Stats(
        ...
        [FromQuery] TimeRangeOnlyQueryRequestModel requestModel)
    {
        ...
        // data
        var result = await _eventDispatcher.Get(new GetAzureLogAnalyticsWorkspaceStatsQuery(
            platformTenantId,
            ValueHelper.ThrowIfNull(requestModel.From),
            ValueHelper.ThrowIfNull(requestModel.To)));

        return OkMapIfNotNull<SecurityPortalAzureLogAnalyticsWorkspaceStatsModel>(_mapper, result);
    }
